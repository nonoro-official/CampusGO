  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return eventAsync.when(
      ...
      data: (updatedEvent) {
        ...
        final hasJoined = businessId != null && currentEvent.attendingBusinessIds.contains(businessId);
          ...
          body: SingleChildScrollView(
            ...
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 10),
                if (currentEvent.attendingBusinessIds.isEmpty)
                  const Text("No vendors attending yet.")
                else
                  ListView.builder(
                    shrinkWrap: true,
                    physics: const NeverScrollableScrollPhysics(),
                    itemCount: currentEvent.attendingBusinessIds.length,
                    itemBuilder: (context, index) {
                      final bId = currentEvent.attendingBusinessIds[index];
                      return FutureBuilder<BusinessModel?>(
                        future: businessService.getBusiness(bId),
                          ...
                          if (snapshot.hasData && snapshot.data != null) {
                            ...
                            return ListTile(
                              ...
                              leading: CircleAvatar(
                                backgroundImage: business.imageUrl != null ? NetworkImage(business.imageUrl!) : null,
                                child: business.imageUrl == null ? const Icon(Icons.store) : null,
                              ),
                ...
                if (isVendor && businessId != null && !currentEvent.isEnded)
                  SizedBox(
                    width: double.infinity,
                    child: ElevatedButton(
                      onPressed: () async {
                        if (hasJoined) {
                          await eventService.leaveEvent(currentEvent.id, businessId);
                        } else {
                          await eventService.joinEvent(currentEvent.id, businessId);
                        }
                      },
