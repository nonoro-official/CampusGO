...
void showListingModal({
  ...
  ModalContainer.show(
    context: context,
    child: _ListingModal(businessId: businessId, product: product),
  );
}
...
class _ListingModal extends ConsumerStatefulWidget {
  ...
  const _ListingModal({required this.businessId, this.product});
...
class _ListingModalState extends ConsumerState<_ListingModal> {
      ...
      if (isExisting) {
        try {
          await ref
              ...
              .removeCategoryFromBusiness(widget.businessId, cat);
          if (mounted) {
            setState(() {
              categories.remove(cat);
            });
  ...
  Widget build(BuildContext context) {
    ...
    final productsAsync = ref.watch(vendorProductsProvider(widget.businessId));
